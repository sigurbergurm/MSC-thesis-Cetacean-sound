...
Currently, the device can sample signals up to 16 bits resolution and record up to 300ksps which should allows it to record signals up to 150kHz until the memory of the SD card is full.
With some further development, it will be shown that the proposed device will be usable for cetacean research and can aid in the conservation efforts.

}  
% ipsum generaes text text
{

Með auknum áhrifum mannfólks á vilt dýr, þá verður eykst mikilvægi þess að fylgjast breytingum í hegðun og búsvæðum dýranna.
Einnig þarf þá að reyna að finna þá áhrifavalda sem trufla dýrin, til að annaðhvort minnka áhrifa þessa eða eyða þeim algjörlega.
...
Þessi ritgerð er hluti af stærra verkefni þar sem tillagan er að nota sjóbauja sem getur framleitt rafmagn fyrir orku litlum raftækjum sérhannaðar í hvalahljóðs upptöku og getur síðan send þau gögn á land, en ritgerðin mun fjalla sérstaklega um hljóðupptöku þátt sjóbaujunar.
...
Tækið notar forritalega bið blokk til þessa að kveikja á hljóðrænum-stafrænum umbreyti til að taka mæligildi og notar síðan beinan minnisaðgang til þess að geyma þau gildi í biðminni sem seinna skrifar gögnin á SD kort.
Eins og er getur tækið tekið upp merki með allt að 16bita upplausn á 300ksps, sem gerir því fræðilega kleift að taka upp merki allt að 150kHz þar til minni SD kortsins klárast.
Með aðeins meiri þróun, verður sýnt fram á að tækið sem lagt er til er hægt að nota í að hjálpa hvala rannsökuðum við rannsóknir og getur hjálpað í þágu náttúrverndar.
...
\include{texts/introduction}%%RUM: Introduction
\include{texts/methods}%%RUM: "Methods"
%\part{The Second Part}
\include{texts/results}%%RUM: "Results"
\include{texts/discussion}%%RUM: "Discussion"
...
\include{texts/tableApp}

\include{texts/CodeExplanation}
