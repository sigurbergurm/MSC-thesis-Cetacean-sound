...
With ever increasing pressure on wildlife caused by humans, it becomes ever so more important to monitor the changes that might occur in the behaviour and habitats of effected animals.
As well as identifying the factors causes disturbances to the animal, in order to minimize or eliminate them.
Therefor creating new or improving old methods for researching and monitoring the impacts on the animals, which is crucial for the preservation of wildlife.
...
The proposed recording device consists of the Teensy 3.5, Aquarian H1a hydrophone which is able to record vocalization signals up to 100kHz.
The device has a low power consumption of 0.33W and a low cost of 190\$.
The device uses a programmable delay block in order to trigger the built in analog to digital converter to take measurements and direct memory access is used to transfer the converted digital value to a buffer which later writes the data onto a SD card.
Currently the device can sample signals at 16 bits resolution is able to record up to 300ksps which should allow it to record signals up to 150kHz.
This thesis will show that the proposed device is capable of aiding cetacean researchers and could potentially aid with conservation efforts.
...
Tækið notar lítið rafmagn eða um 0.33W og kostar lítið eða 190\$.
